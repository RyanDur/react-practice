...
import {selectable} from '../Selectable/store';
...
describe('Table state', () => {
  ...
  //     expect(state().fancy.rows).toEqual(initialTableState.rows);
  //   });
  // });
  //
  ...
  //       expect(state().fancy.columns).toEqual({
  //         active: [
  //           'bar',
  //           'baz',
  //           'bob',
  ...
  //
  //     // expect(state().fancy.rows.filter((row: CheckedRow) => !row.selected)[0].name).toBe('Harrison');
  //   });
  //
  //   xit('should set undefined to true', () => {
  //     store.dispatch({type: fancyAction.TOGGLE_CHECKED, row: 'Harrison'});
  //     // expect(state().fancy.rows.filter((row: Rows) => row.name === 'Harrison')[0].selected).toBeTruthy();
  //   });
  // });
});
